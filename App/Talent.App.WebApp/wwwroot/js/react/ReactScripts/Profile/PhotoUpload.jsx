...
export default class PhotoUpload extends Component {
    ...
    //            this.setState({ imageSrc  :res})
    //        }.bind(this),
          
    //    });
    //}
    saveUpload() {
        const data = this.props.imageId
    ...
    render() {
        ...
        for (let i = 0; i < this.state.currentNoOfFiles; i++) {
            if (this.state.imageSrc[i] != null) {
                showProfileImg.push(<span><img style={{ height: 112, width: 112, borderRadius: 55 }} className="ui small" src={this.state.imageSrc[i]} alt="Image Not Found" /><button type="button" onClick={this.fileUploadHandler} className="ui black button" value={i} imageid={this.state.imageId[i]}>Upload</button></span>);
            }
        }
